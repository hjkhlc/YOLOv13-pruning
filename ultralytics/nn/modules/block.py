...
# new code
import logging  # + 文件顶部添加（~行10后）
LOGGER = logging.getLogger(__name__)  # + 添加
# new code end
...
class AdaHyperedgeGen(nn.Module):
    ...
    def __init__(self, node_dim, num_hyperedges, num_heads=4, dropout=0.1, context="both"):
        ...
        # new code ：重要性统计（移动平均，init=0） & 梯度钩子（任务2）
        self.importance_stats = nn.Parameter(torch.zeros(num_hyperedges))
        self.register_backward_hook(self._grad_hook)

        # new code end
    
    # new code ：任务2：计算梯度norm，累加到stats (权重0.5)
    # 任务2：计算梯度norm，累加到stats (权重0.5)
    def _grad_hook(self, module, grad_input, grad_output):
        
        if len(grad_output) > 0:
            grad = grad_output[0]
            if grad is not None:
                imp = torch.norm(grad, p=2).mean().item() * 0.5  # .item() scalar
                with torch.no_grad():
                    self.importance_stats.data += imp  # scalar cum加
        return None  # 修复：返回None (PyTorch backward hook标准)
    # end
    # new code end
    ...
    def forward(self, X):
        ...
        # new code : 任务1：L1/L2/最大激活重要性，移动平均 (alpha=0.99)
        with torch.no_grad():
            l1_imp = torch.norm(A, p=1, dim=1).mean(dim=0)  # L1 per edge
            l2_imp = torch.norm(A, p=2, dim=1).mean(dim=0)  # L2 per edge
            max_imp = A.max(dim=1)[0].mean(dim=0)  # Max activation per edge
            edge_imp = (l1_imp + l2_imp + max_imp) / 3  # 平均
            alpha = 0.99
            self.importance_stats.data = alpha * self.importance_stats.data + (1 - alpha) * edge_imp.detach()
        # new code end
...
class AdaHGConv(nn.Module):
    ...
    def __init__(self, embed_dim, num_hyperedges=16, num_heads=4, dropout=0.1, context="both"):
        ...
        # new code : 任务4：剪枝mask (bool tensor)
        # self.prune_mask = nn.Parameter(torch.ones(num_hyperedges, dtype=torch.bool))
        self.prune_mask = nn.Parameter(torch.ones(num_hyperedges, dtype=torch.float32))
        # new code end
    ...
    def forward(self, X):
        ...
        # new code
        # + 任务4：mask A (V2E)
        # + 修复：V2E mask (1,1,E) 广播到 A (B,N,E)
        mask_v2e = self.prune_mask.unsqueeze(0).unsqueeze(0).float()  # (1,1,E)
        A_masked = A * mask_v2e
        He = torch.bmm(A_masked.transpose(1, 2), X) 
        # + 任务3：幅度重要性 (He后softmax归一化，累加到gen.stats，轻权0.1)
        with torch.no_grad():
            amp = torch.norm(He, p=2, dim=-1).mean(dim=0)  # Per edge amplitude (E,)
            amp_soft = F.softmax(amp, dim=0)
            self.edge_generator.importance_stats.data += amp_soft.detach() * 0.1
        He = self.edge_proj(He)
        # + 任务4：mask He (E2V)
        # + 修复：E2V mask (1,E,1) 广播到 He (B,E,D)
        mask_e2v = self.prune_mask.unsqueeze(0).unsqueeze(-1).float()  # (1,E,1)
        He_masked = He * mask_e2v
        X_new = torch.bmm(A_masked, He_masked)  
        # new code end
...
class C3AH(nn.Module):
    ...
    def __init__(self, c1, c2, e=1.0, num_hyperedges=8, context="both"):
        ...
        # new code
        self.num_hyperedges_orig = num_hyperedges  # + 保存原值
        # new code end
    ...
    # new code : 任务5：动态调整num_hyperedges和mask
    def adjust_prune(self, ratio, keep_indices=None):
        orig_num = self.m.hgnn.edge_generator.num_hyperedges
        keep_num = int(orig_num * (1 - ratio))
        keep_num = max(keep_num, 1)
        if keep_indices is not None and len(keep_indices) >= keep_num:
            self.m.hgnn.prune_mask.data.fill_(0.0)
            self.m.hgnn.prune_mask.data[keep_indices[:keep_num]] = 1.0
        else:
            rand_vals = torch.rand_like(self.m.hgnn.prune_mask.data)
            # ~ 修正：torch.sort 返回 (values, indices)，用 [0] 取 values
            sorted_vals = torch.sort(rand_vals, descending=True)[0]
            threshold = sorted_vals[keep_num - 1] if keep_num > 0 else 0.0
            self.m.hgnn.prune_mask.data = (rand_vals > threshold).float()
        self.m.hgnn.prune_mask.detach_()  # 冻结
    # new code end
...
class FuseModule(nn.Module):
    ...
    def __init__(self, c_in, channel_adjust):
        ...
        self.c = c_in  # new code : target ch after proj
        self.downsample = nn.AvgPool2d(kernel_size=2)
        self.upsample = nn.Upsample(scale_factor=2, mode='nearest')
        # new code : 投影：假设x[0]/x[1] ch=c_in, x[2] ch=2*c_in (YOLOv13设计)
        self.proj0 = nn.Identity()  # high-res -> c
        self.proj1 = nn.Identity()  # mid -> c
        self.proj2 = Conv(2 * c_in, c_in, 1)  # low-res (2x) -> c
        
        # ~ 修改：始终用 3*c_in（匹配 3 输入融合），忽略 channel_adjust（若需 4，可后续扩展）
        self.conv_out = Conv(3 * c_in, c_in, 1) # cat 3*c -> c

    def forward(self, x):
        # ~ 增强 assert：检查通道一致 + cat 后形状
        assert len(x) == 3, f"FuseModule expects 3 inputs, got {len(x)}"
        # assert all(xi.shape[1] == x[0].shape[1] for xi in x), f"Prune shape mismatch in FuseModule: {[xi.shape[1] for xi in x]}"
        # x1_ds = self.downsample(x[0])
        # x3_up = self.upsample(x[2])
        # # ~ 添加：确保 ds/up 后空间匹配 mid
        # assert x1_ds.shape[2:] == x[1].shape[2:], f"Downsample mismatch: {x1_ds.shape[2:]} vs {x[1].shape[2:]}"
        # assert x3_up.shape[2:] == x[1].shape[2:], f"Upsample mismatch: {x3_up.shape[2:]} vs {x[1].shape[2:]}"
        # x_cat = torch.cat([x1_ds, x[1], x3_up], dim=1)  # 192ch for c_in=64
        
        # new code 
        x0_ds = self.proj0(self.downsample(x[0]))
        x1 = self.proj1(x[1])
        x2_up = self.proj2(self.upsample(x[2]))
        assert x0_ds.shape[2:] == x1.shape[2:], f"Downsample mismatch: {x0_ds.shape[2:]} vs {x1.shape[2:]}"
        assert x2_up.shape[2:] == x1.shape[2:], f"Upsample mismatch: {x2_up.shape[2:]} vs {x1.shape[2:]}"
        # 任务7: post-prune mask广播（若有prune_mask，假设从HyperACE传；否则跳过）
        x_cat = torch.cat([x0_ds, x1, x2_up], dim=1)
        if hasattr(self, 'prune_mask') and self.prune_mask is not None:
            b, c, h, w = x_cat.shape
            mask = self.prune_mask.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand(b, -1, h, w)  # [1, num_e, 1, 1] -> [b, c, h, w] but adapt if per-channel
            x_cat = x_cat.masked_fill(~mask.bool(), 0.0)
        # new code end
...
class HyperACE(nn.Module):
    ...
    # new code : 任务6：全局剪枝方法
    def prune_hyperedges(self, ratio=0.2):
        branches = [self.branch1, self.branch2]
        all_stats = torch.stack([b.m.hgnn.edge_generator.importance_stats.detach() for b in branches]).mean(dim=0)
        orig_num = len(all_stats)
        keep_num = int(orig_num * (1 - ratio))
        keep_num = max(keep_num, 1)
        _, keep_idx = torch.topk(all_stats, keep_num)  # ~ 用 keep_num
        for b in branches:
...
# 新增：PruneHyperACE子类（继承HyperACE，支持显式prune）
class PruneHyperACE(HyperACE):
    """
    Pruned variant of HyperACE for structured hyperedge pruning.

    Inherits all functionality from HyperACE and exposes prune_hyperedges for easy integration.
    Designed for dynamic deployment in YOLOv13, reducing FLOPs by masking low-importance hyperedges.

    Args:
        *args: Arguments passed to parent HyperACE.
        **kwargs: Keyword arguments passed to parent HyperACE.

    Methods:
        prune(ratio): Shortcut to prune_hyperedges for compatibility with model-level calls.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def prune(self, ratio=0.2):
        """Shortcut for prune_hyperedges, enabling model-level pruning."""
        self.prune_hyperedges(ratio)


# 新增：compute_importance函数（全局汇总stats，支持DDP）
def compute_importance(model_modules, sync_dist=False):
    """
    Computes aggregated importance scores across HyperACE/PruneHyperACE modules.

    Aggregates 'importance_stats' from all matching modules using mean pooling.
    Optional DDP synchronization for multi-GPU training.

    Args:
        model_modules (nn.Module or list): Model or list of modules to traverse.
        sync_dist (bool): If True, synchronize stats across GPUs using all_reduce.

    Returns:
        (torch.Tensor): Aggregated importance tensor (shape: [num_hyperedges]).

    Raises:
        ValueError: If no HyperACE modules found.

    Example:
        >>> from ultralytics.nn.modules.block import compute_importance
        >>> import torch.distributed as dist
        >>> stats = compute_importance(model)
        >>> if sync_dist: dist.all_reduce(stats, op=dist.ReduceOp.AVG)
    """
    import torch.distributed as dist  # 新增：延迟导入，避免非 DDP 环境错误
    if isinstance(model_modules, nn.Module):
        modules = model_modules.modules()
    else:
    ...
    for m in modules:
        if isinstance(m, (HyperACE, PruneHyperACE)):
            if hasattr(m, 'branch1') and hasattr(m.branch1.m, 'hgnn') and hasattr(m.branch1.m.hgnn, 'edge_generator'):
                # 平均两个分支的stats
                branch_stats = torch.stack([
                    m.branch1.m.hgnn.edge_generator.importance_stats.detach(),
                    m.branch2.m.hgnn.edge_generator.importance_stats.detach()
                ]).mean(dim=0)
                stats_list.append(branch_stats)

    if not stats_list:
        raise ValueError("No HyperACE/PruneHyperACE modules found for importance computation.")

    agg_stats = torch.stack(stats_list).mean(dim=0)  # 全局平均

    if sync_dist:
        try:
            if dist.is_available() and dist.is_initialized():
                dist.all_reduce(agg_stats, op=dist.ReduceOp.AVG)
                agg_stats /= dist.get_world_size()
        except RuntimeError:  # 新增：捕获 DDP 未初始化错误
            LOGGER.warning("DDP not initialized, skipping stats sync.")

    return agg_stats

# 更新__all__（在block.py末尾添加）
__all__ = __all__ + ("PruneHyperACE", "compute_importance")