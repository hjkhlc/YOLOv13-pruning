...
from ultralytics.nn.modules import HyperACE  # 新增：导入HyperACE以支持isinstance检查
...
class Model(nn.Module, PyTorchModelHubMixin, repo_url="https://github.com/ultralytics/ultralytics", pipeline_tag="object-detection", license="agpl-3.0"):
    ...
    # new code ：任务13 - 模型级全局剪枝方法
    def prune(self, ratio: float = 0.2) -> "Model":
        """
        Performs global pruning on HyperACE modules in the model.

        This method traverses all submodules of self.model and calls prune_hyperedges(ratio) on each HyperACE
        instance. It supports structured pruning of hyperedges based on importance stats, reducing FLOPs while
        maintaining performance. Pruning is applied only to HyperACE branches (e.g., in YOLOv13 backbone/neck).

        Args:
            ratio (float): Pruning ratio (0.0-1.0). Defaults to 0.2 (prune 20% hyperedges).

        Returns:
            (Model): The pruned model instance (self), enabling method chaining.

        Raises:
            TypeError: If self.model is not a PyTorch nn.Module.

        Examples:
            >>> from ultralytics import YOLO
            >>> model = YOLO("yolov13n.pt")
            >>> model.prune(ratio=0.3)  # Prune 30% hyperedges
            >>> metrics = model.val()  # Validate post-pruning
        """
        self._check_is_pytorch_model()  # 确保模型为PyTorch类型
        pruned_count = 0
        for m in self.model.modules():
            if isinstance(m, HyperACE):
                m.prune_hyperedges(ratio)
                pruned_count += 1
        from ultralytics.utils import LOGGER  # 延迟导入以避免循环
        LOGGER.info(f"Pruned {pruned_count} HyperACE modules with ratio={ratio:.2f}")
        return self
